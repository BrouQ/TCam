...
enable_ref_t = True
# file_ref_t = "/sys/bus/w1/devices/28-00000c1370bd/w1_slave" # integrac
file_ref_t = "/sys/bus/w1/devices/28-02039245b536/w1_slave" # sonda kabel
...
while True:
    ...
    mujtext = "cam: {:.3f}, ref: {:.3f}, diff: {:.3f}".format(roi_t, ref_t/1000, roi_t-ref_t/1000)
    cv2.putText(
      img_v, #numpy array on which text is written
      mujtext, #text
      (10,50), #poimport matplotlib.pyplot as pltsition at which writing has to start
    ...
    # logovani
    log_trigger+=1
    f.write("{}; {:.3f}; {:.3f}; {:.3f}\n".format(time.time(), roi_t, ref_t/1000, roi_t-ref_t/1000))
    if log_trigger >= 100:
        log_trigger=0
        f.close()
        f = open("log.csv", "a")
